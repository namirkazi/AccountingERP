...
export default function SavedCapitalVoucher({
  ...
  const handleSavePdf = async () => {
    if (!voucherRef.current || isGeneratingPdf) {
      return;
    }
    ...
    try {
      ...
      const fileName = String(voucherNumber || "Capital-Contribution-Voucher")
        .replace(/[^a-z0-9/_-]+/gi, "-")
        .replace(/\//g, "-");
      ...
      console.error("Failed to generate capital voucher PDF:", error);
    } finally {
    ...
    <div className={styles.savedVoucherOverlay}>
      ...
      <div className={styles.savedVoucherToolbar}>
        <div>
          ...
          <strong>Capital Transfer Voucher</strong>
        </div>

        <div className={styles.savedVoucherActions}>
          <button
            type="button"
            className={styles.printVoucherButton}
          ...
          <button
            type="button"
            className={styles.closeVoucherButton}
      ...
      <div ref={voucherRef} className={styles.printableVoucherPaper}>
