...
function numberToWords(value) {
  ...
  if (remaining >= 1000000) {
    ...
    words += numberToWordsBelowThousand(millions) + " Million";
  ...
  if (remaining > 0) {
    words += numberToWordsBelowThousand(remaining);
  }

  words += wholeDirhams === 1 ? " Dirham" : " Dirhams";
...
function getItemAmount(item) {
  ...
  return getItemQuantity(item) * getItemRate(item);
}
...
export default function PrintableVoucher({
  ...
  const companyLogo =
    ...
    getCompanyLogoUrl(clean(company?.logo) || clean(company?.logo_url));
  ...
  const partyName = getPartyName(party, selectedPaymentBill);

  const partyAddress = getPartyAddress(party, selectedPaymentBill);

  const partyPhone = getPartyPhone(party, selectedPaymentBill);

  const partyEmail = getPartyEmail(party, selectedPaymentBill);

  const partyTrn = getPartyTrn(party, selectedPaymentBill);
  ...
  const ourBillNumber =
    clean(voucherNumber) ||
    clean(isReceipt ? selectedReceiptBill?.invoice_number : "") ||
    clean(selectedPaymentBill?.voucher_number) ||
    clean(selectedPaymentBill?.voucher_no) ||
    "—";
  ...
  const expenseItems = normalizeItems(items);

  const calculatedItemSubtotal = expenseItems.reduce(
    (sum, item) => sum + getItemAmount(item),
    0,
  );

  const subtotal =
    calculatedItemSubtotal > 0 ? calculatedItemSubtotal : Number(amount || 0);
  ...
  const paymentTotal = Number(paymentAmount || 0);

  const expenseTotal = Number(totalAmount || 0);
  ...
  if (isPayment) {
    ...
    finalAmount = expenseTotal || Math.max(0, subtotal - discount + vat);
  } else {
  ...
  if (isPayment) {
    defaultDescription = partyName;
  }

  if (isReceipt) {
    defaultDescription = "Receipt from Customer";
  }

  if (isSale) {
    defaultDescription = "Sale";
  }

  const description = clean(narration) || defaultDescription;
    ...
    <div className={styles.voucher}>
      ...
      <header className={styles.documentHeader}>
        ...
        <div className={styles.partyInformation}>
          <div className={styles.partyInformationLabel}>
            {isSale || isReceipt ? "CUSTOMER" : "SUPPLIER"}
          </div>
          ...
          {partyPhone && <span>Tel: {partyPhone}</span>}

          {partyEmail && <span>{partyEmail}</span>}

          {partyTrn && <span>TRN: {partyTrn}</span>}
        </div>
      </header>
      ...
      <section className={styles.titleSection}>
        <div>
          <span className={styles.documentEyebrow}>ACCOUNTING DOCUMENT</span>

          <h2>{documentTitle}</h2>
        </div>

        <div className={styles.documentStatus}>ORIGINAL</div>
      </section>
      ...
      <section
        className={`${styles.documentDetails} ${
          isPayment ? styles.paymentDocumentDetails : ""
        }`}
      >
        <div>
          <span>DATE</span>
        ...
        <div>
          ...
          <strong>{ourBillNumber}</strong>
        </div>

        {!isSale && !isReceipt && (
          <div>
            <span>
              {isPayment || isReceipt ? "AGAINST BILL" : "REFERENCE NO."}
            </span>
      ...
      {isExpense && (
        <section className={styles.totalsSection}>
          <div className={styles.totalRow}>
            ...
            <strong>AED {formatAmount(subtotal)}</strong>
          </div>

          <div className={styles.totalRow}>
            <span>Discount</span>

            <strong>- AED {formatAmount(discount)}</strong>
          </div>
          ...
          <div className={styles.totalRow}>
            ...
            <strong>
              AED {formatAmount(Math.max(0, subtotal - discount))}
            </strong>
          </div>
          ...
          <div className={styles.totalRow}>
            ...
            <strong>AED {formatAmount(vat)}</strong>
          </div>

          <div className={styles.grandTotalRow}>
            <span>TOTAL</span>

            <strong>AED {formatAmount(finalAmount)}</strong>
          </div>
        </section>
      )}
      ...
      {isReceipt && (
        <section className={styles.paymentSummary}>
          <div>
            ...
            <strong>
              AED {formatAmount(selectedReceiptBill?.amount || 0)}
            </strong>
          </div>
          ...
          <div>
            ...
            <strong>
              AED {formatAmount(selectedReceiptBill?.received_amount || 0)}
            </strong>
          </div>
          ...
          <div className={styles.paymentBalance}>
            ...
            <strong>
              ...
              {formatAmount(
                Math.max(
                  0,
                  Number(selectedReceiptBill?.outstanding_amount || 0) -
                    finalAmount,
                ),
              )}
            </strong>
      ...
      <section className={styles.amountWordsSection}>
        <span>AMOUNT IN WORDS</span>

        <strong>{numberToWords(finalAmount)}</strong>
      </section>
